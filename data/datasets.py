...
def recursively_read(rootdir, must_contain, exts=["png", "jpg", "JPEG", "jpeg"]):
    ...
    # class_labels = ['tvmonitor', 'horse']
    # class_labels = ['tvmonitor', 'cat']
    # class_labels = ['car', 'chair']
    # class_labels = ['tvmonitor', 'sofa']
    # class_labels = ['sofa', 'chair']
    # class_labels = ['sofa', 'diningtable']
    class_labels = ['tvmonitor', 'diningtable']
    # class_labels = ['chair', 'diningtable']
    # class_labels = ['chair', 'bus']
    for r, d, f in os.walk(rootdir, followlinks=True):
        for file in f:
            if (file.split('.')[1] in exts)  and  (must_contain in os.path.join(r, file)) :
                # and r.split("/")[-2] in class_labels:
                if "ForenSynths" in rootdir:
...
class RealFakeDataset(Dataset):
    ...
    #     return img, label
    def __getitem__(self, idx):
        # 从给定 idx 开始，向后寻找第一个能正常打开的图片
        for offset in range(len(self.total_list)):
            cur_idx = (idx + offset) % len(self.total_list)
            img_path = self.total_list[cur_idx]
            try:
                img = Image.open(img_path).convert("RGB")
                label = self.labels_dict[img_path]   # 标签也放在 try 里，防止路径不在字典
                img = self.transform(img)
                return img, label
            except (UnidentifiedImageError, OSError, KeyError) as e:
                print(f"跳过坏图: {img_path} | {e}")
                continue
        # 如果整个数据集都坏了（极端情况），抛出一个明确的异常
        raise RuntimeError("数据集所有图片都无法读取")
